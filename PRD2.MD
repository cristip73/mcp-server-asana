...
- **Implementation Details**:
  - Create a new error handling utility in `src/utils/error-utils.ts`:
    ```typescript
    /**
    ...
    // Map of known error patterns to friendly messages
    const errorMappings: ErrorMapping[] = [
      {
        pattern: /not a valid task/i,
        friendlyMessage: "The task ID you provided doesn't exist or is inaccessible",
        recoverySteps: [
          "Check that the task ID is correct",
          "Verify that you have access to this task",
          "Try getting the task with asana_get_task to confirm access"
        ]
      },
      {
        pattern: /not a valid user/i,
        friendlyMessage: "The user ID you provided doesn't exist or is inaccessible",
        recoverySteps: [
          "Check that the user ID is correct",
          "Verify that the user is part of your organization"
        ]
      },
      {
        pattern: /not a valid project/i,
        friendlyMessage: "The project ID you provided doesn't exist or is inaccessible",
        recoverySteps: [
          "Check that the project ID is correct",
          "Verify that you have access to this project",
          "Try getting the project with asana_get_project to confirm access"
        ]
      },
      {
        ...
        recoverySteps: [
          "Ask a workspace admin to give you appropriate permissions",
          "Check if the resource belongs to a different workspace"
        ]
      },
      {
        pattern: /custom field .* is not present/i,
        friendlyMessage: "The custom field you're trying to update doesn't exist on this task",
        recoverySteps: [
          "Use asana_get_task with opt_fields: 'custom_fields' to see available fields",
          "Make sure the custom field GID is correct",
          "Verify that the field is actually added to the task's container (project)"
        ]
      },
      {
        pattern: /A valid enum option is required for this custom field/i,
        friendlyMessage: "The value you provided for an enum custom field is not valid",
        recoverySteps: [
          "Use asana_get_task with opt_fields: 'custom_fields' to see available enum options",
          "Make sure you're using the enum_option.gid, not the name of the option"
        ]
      },
      {
        pattern: /section not found/i,
        friendlyMessage: "The section ID you provided doesn't exist or is inaccessible",
        recoverySteps: [
          "Check that the section ID is correct",
          "Use asana_get_project_sections to get valid section IDs for your project"
        ]
      }
    ];
    
    /**
     * Formats an API error into a user-friendly message with recovery steps
     * @param error The error object from the Asana API
     * @returns User-friendly error message
     */
    export function formatApiError(error: any): any {
      ...
      for (const mapping of errorMappings) {
        if (mapping.pattern.test(errorMessage)) {
          return {
            error: mapping.friendlyMessage,
            original_error: errorMessage,
            recovery_steps: mapping.recoverySteps || []
          };
        }
      }
      ...
      return {
        error: `An error occurred with the Asana API: ${errorMessage}`,
        original_error: errorMessage,
        recovery_steps: [
  ...
  - Update the `src/tool-handler.ts` to use the enhanced error messages:
    ```typescript
    // Add import
    import { formatApiError } from './utils/error-utils.js';
    
    // In the try/catch block of the handle method
    try {
      // Existing code to process the tool call
      // ...
    } catch (error) {
  ...
  - Add detailed error documentation to the README:
    ```markdown
    ...
    - The original error from the Asana API for reference
    - Suggested recovery steps to help resolve the issue
    
    Example error response:
    
    ```json
    {
      "error": "The custom field you're trying to update doesn't exist on this task",
      "original_error": "Custom field 123456 is not present on task 789012",
      "recovery_steps": [
        "Use asana_get_task with opt_fields: 'custom_fields' to see available fields",
        "Make sure the custom field GID is correct",
        "Verify that the field is actually added to the task's container (project)"
    ...
    1. **Task/Project/User not found**
       - Check that the ID is correct
       - Verify you have access to the resource
    
    2. **Permission errors**
       - Make sure you have the right permissions in the workspace
       - Check if you're trying to modify a resource you don't have write access to
    
    3. **Custom field errors**
       - For enum fields, use the enum_option.gid, not the name
       - Verify the custom field exists on the task's parent project
       - Check that the value type matches the custom field type
    ```
  
  - **AI usage guidance to include:** "When you encounter an error, look at the recovery_steps in the response for specific guidance on how to fix the issue. For custom field errors, always use asana_get_task first to see the available fields and their correct formats."
